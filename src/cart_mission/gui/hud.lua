    ...
    function new_hud(params)
        ...
        return {
            ...
            _draw = function(game)
                ...
                line(mission_progress_x, 4, mission_progress_x, 3 + mission_progress_h, _color_13_lavender)
                ship_indicator._draw(xy.minus(-4, 77 + game.mission_progress_fraction() * mission_progress_h - 3))
                xy = _xy(ceil(_gaw + 5 - slide_in_offset.xy.x), ceil(_vs - 16 - slide_in_offset.xy.y))
                ...
                if game.boss_health_max then
                    ...
                    line(_gaox + boss_health_bar_margin + 2, boss_health_bar_margin + 2, _gaox + _gaw - boss_health_bar_margin - 3, boss_health_bar_margin + 2, _color_14_mauve)
                    if health_fraction > 0 then
                        line(_gaox + boss_health_bar_margin + 2, boss_health_bar_margin + 1, _gaox + boss_health_bar_margin + 2 + flr(health_fraction * _gaw - 2 * boss_health_bar_margin - 4) - 1, boss_health_bar_margin + 1, _color_8_red)
                    end
                end
            end
        }

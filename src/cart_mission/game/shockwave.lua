    ...
    function new_shockwave(center_xy, speed)
        ...
        function draw_circle(r)
            if r == mid(r_min, r, r_max) then
                oval(x - r, y - r, x + r, y + r, _color_6_light_grey)
            end
        end
        
        function draw_negative_ring(r_outer, r_inner)
            ...
            for dy = -r_outer, r_outer do
                ...
                sspr(x - dx_outer, sy, dx_outer - dx_inner, 1, x - dx_outer, sy)
                sspr(x + dx_inner, sy, dx_outer - dx_inner, 1, x + dx_inner, sy)
            end
            pal()

...
function new_player(params)
    ...
    local jet_sprite_visible = new_animated_sprite(4, 20, split("0,0,0,0,4,4,4,4"), 9)
    ...
    function create_single_bullet()
        return {new_player_bullet(xy.plus(0, -4))}
    end
    
    function create_triple_bullets()
        return {
    ...
    return {
        ...
        set_movement = function(left, right, up, down, fast_movement)
            ...
            xy = _xy(mid(w / 2 + 1, xy.x + x_diff, _gaw - w / 2 - 1), mid(h / 2 + 1, xy.y + y_diff, _gah - h / 2 - 1))
        end,
        fire = function(fast_shoot, triple_shoot)
            on_bullets_spawned.invoke_if_ready(triple_shoot and fast_shoot and 10 or 16 or fast_shoot and 8 or 12, triple_shoot and create_triple_bullets or create_single_bullet)
        end,
        trigger_shockwave = function()
            on_shockwave_triggered.invoke_if_ready(6, create_shockwave)
        end,

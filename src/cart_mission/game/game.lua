...
function new_game(health, shockwave_charges, fast_movement, fast_shoot, triple_shoot, score)
    ...
    local player = new_player {
        ...
        on_destroyed = function(collision_circle)
            ...
            _add_all(explosions, new_explosion(collision_circle.xy, collision_circle.r), new_explosion(collision_circle.xy, 2 * collision_circle.r, 4 + flr(rnd(8))), new_explosion(collision_circle.xy, 3 * collision_circle.r, 12 + flr(rnd(8))))
        end
    }
    --
    ...
    function handle_powerup(powerup_type, powerup_xy)
        ...
        _sfx_play(has_effect and _sfx_powerup_picked or _sfx_powerup_no_effect, has_effect and 2 or nil)
    end
            ...
            for enemy_cc in all(enemy.collision_circles()) do
                for shockwave in all(shockwaves) do
                    ...
                    if not enemy.has_finished() and not shockwave.has_finished() and shockwave_enemy_hits[combined_id] < 8 then
                        if _collisions.are_colliding(shockwave, enemy_cc, {ignore_gameplay_area_check = true}) then
                            enemy.take_damage(2)
            ...
            for boss_cc in all(boss.collision_circles()) do
                for shockwave in all(shockwaves) do
                    ...
                    if not boss.has_finished() and not shockwave.has_finished() and shockwave_enemy_hits[combined_id] < 8 then
                        if _collisions.are_colliding(shockwave, boss_cc, {ignore_gameplay_area_check = true}) then
                            boss.take_damage(2)
                            shockwave_enemy_hits[combined_id] = shockwave_enemy_hits[combined_id] + 1
                        end
                    end
        ...
        boss = new_boss {
            ...
            on_destroyed = function(collision_circles, score_to_add)
                ...
                for cc in all(collision_circles) do
                    ...
                    _add_all(explosions, new_explosion(xy, .8 * r), new_explosion(xy, 1.4 * r, 4 + flr(rnd(44)), function()
                        _sfx_play(_sfx_destroy_boss_final_2)
                    end), new_explosion(xy, 1.8 * r, 12 + flr(rnd(36)), function()
                        _sfx_play(_sfx_destroy_boss_final_3)
                    end), new_explosion(xy, 3.5 * r, 30 + flr(rnd(18))), new_explosion(xy, 5 * r, 50 + flr(rnd(6))))
                end
            end
        }
    end
    ...
    function game._update()
        ...
        _flattened_for_each({level}, shockwaves, player_bullets, enemy_bullets, {player}, enemies, {boss}, powerups, explosions, {camera_shake_timer}, floats, function(game_object)
            game_object._update()
        end)
        if player then
            handle_collisions()
    ...
    function game._draw()
        ...
        _flattened_for_each({level}, enemies, {boss}, player_bullets, enemy_bullets, {player}, powerups, explosions, floats, shockwaves, function(game_object)
            game_object._draw()
        end)
        -- some enemies are placed on a ground and have collision circle smaller than a sprite, therefore have to be drawn before a player and bullets
        ...
        if camera_shake_timer.ttl > 0 then
            ...
            camera(rnd(factor) - .5 * factor, rnd(factor) - .5 * factor)
        end
    end
    ...
    function game._post_draw()
        ...
        _flattened_for_each(shockwaves, player_bullets, enemy_bullets, enemies, powerups, explosions, floats, function(game_object, game_objects)

                ...
                for lane = 1, 12 do
                    ...
                    if enemy_map_marker then
                        add(result, {
                            ...
                            xy = _xy((lane - .5) * _ts, _vs - _ts - (spawn_distance - min_visible_distance + .5) * _ts).plus(enemy_offset)
                        })
                    end
                end
            end
            ...
            if phase == "main" then
                for distance = flr(min_visible_distance), ceil(max_visible_distance) do
                    for lane = 1, 12 do
                        ...
                        if fg_tile then
                            spr(fg_tile, _gaox + (lane - 1) * _ts, _vs - flr((distance - min_visible_distance + 1) * _ts))
                        end
                    end
                end
            end

...
function new_movement_to_target_factory(params)
    ...
    return function(start_xy)
        ...
        local function xy()
            return _xy(_easing_lerp(start_xy.x, params.target_x or start_xy.x, easing_fn(timer.passed_fraction())), _easing_lerp(start_xy.y, params.target_y or start_xy.y, easing_fn(timer.passed_fraction())))
        end

...
function new_screen_select_mission(selected_mission)
    ...
    local jet_sprite = new_animated_sprite(4, 4, split("0,0,0,0,4,4,4,4"), 8)
    function mission_button_xy_wh(mission_number)
    ...
    function draw_back_button()
        ...
        -- button shape
        sspr(selected and 35 or 36, 12, 1, 12, button_xy1.x - 1, button_xy1.y - 1, button_wh.x + 2, 12)
        -- button text
    ...
    function draw_mission_button(mission_number)
        ...
        -- draw button shape
        sspr(selected and 38 or 39, 12, 1, 19, button_xy1.x - 1, button_xy1.y - 1, button_wh.x + 2, 19)
        -- draw level sample
        local sy = 80 + mission_number - 1 * 16
        sspr(0, selected and sy or sy - 48, button_wh.x, button_wh.y, button_xy1.x, button_xy1.y)
        if mission_number > 1 then
            -- draw WIP info
            _centered_print("under \-fdevelopment", button_xy1.y + 2, selected and _color_7_white or _color_6_light_grey, selected and _color_9_dark_orange or _color_13_lavender)
        end
        -- draw label
        print("mission " .. mission_number, button_xy1.x, button_xy2.y + 4, selected and _color_7_white or _color_13_lavender)
        -- draw "x" button press incentive and its label
        if selected then
            print("start", button_xy2.x - 37, button_xy2.y + 4, _color_7_white)
            local sprite = _alternating_0_and_1() == 0 and x_sprite or x_pressed_sprite
    ...
    function screen._post_draw()
        ...
        if fade_out.has_finished() then
            ...
            _load_mission_cart(selected_mission, _health_default, _shockwave_charges_default, false, false, false, 0)
        end
    end
